...
Private Function GetMsgOutOfColumnRange(ByVal num As Long) As String
	GetMsgOutOfColumnRange = _
		"[" & Me.TableName & "] 列範囲を逸脱する値が入力されました" & vbCrLf & _
		"範囲 : (" & CStr(Me.BeginColumnNum) & " to " & CStr(Me.EndColumnNum) & ") 入力値 : " & CStr(num)
End Function
...
Private Function GetMsgOutOfRowRange(ByVal num As Long) As String
	GetMsgOutOfRowRange = _
		"[" & Me.TableName & "] 行範囲を逸脱する値が入力されました" & vbCrLf & _
		"範囲 : (" & CStr(Me.BeginRowNum) & " to " & CStr(Me.EndRowNum) & ") 入力値 : " & CStr(num)
End Function
...
Private Function GetMsgDataAccessError(ByVal column As Long, ByVal row As Long, ByVal value As Variant) As String
	...
	GetMsgDataAccessError = err_msg & _
		"[Size]     : (" & Cstr(Me.ColumnSize) & ", " & CStr(Me.RowSize) & ")" & vbCrLf & _
		"[Input]    : (" & Cstr(column) & ", " & CStr(row) & ")"
End Function
...
Function ResetColumnSize(ByVal size As Long)
	...
	If (size < 1) Then Call ThrowArgumentOutOfRangeException(Me, FUNCTION_NAME, "size", "[" & CStr(size) & " < 1] : 入力値が範囲外です")
...
Function Copy(ByRef table_data As TableDataBase)
	...
	If (table_data Is Nothing) Then	Call ThrowArgumentNullException(Me, FUNCTION_NAME, "table_data", "コピーに失敗しました")
	If (table_data.Name <> Me.Name) Then Call ThrowArgumentException(Me, FUNCTION_NAME, "table_data", "[" & table_data.Name & "] 対象外のオブジェクトです")
...
Public Function InsertColumn(ByVal insert_size As Long, Optional ByVal insert_pos = -1, Optional ByVal is_preserve As Boolean = True)
	...
	If (insert_size = 0) Then Exit Function
	If (insert_size < 0) Then Call ThrowArgumentException(Me, FUNCTION_NAME, "insert_size", "[" & Me.TableName & "] 追加数が少なすぎます")
	If (IsOutOfColumnRange(insert_pos)) Then insert_pos = Me.EndColumnNum
	...
	On Error Goto CatchErr
		...
		If ((insert_pos < Me.EndRowNum) And (is_preserve)) Then Set f_all_title_area.Table = old_table_title.Table
	Exit Function
	CatchErr:
		Call RethrowException(Me, FUNCTION_NAME)
End Function
...
Function InsertRow(ByVal insert_size As Long, Optional ByVal insert_pos = -1, Optional ByVal is_preserve As Boolean = True)
	...
	If (insert_size = 0) Then Exit Function
	If (insert_size < 0) Then Call ThrowArgumentException(Me, FUNCTION_NAME, "insert_size", "[" & Me.TableName & "] 追加数が少なすぎます")
	If (IsOutOfRowRange(insert_pos)) Then insert_pos = Me.EndRowNum
...
Public Function AddColumn(ByRef add_data As TableDataColumn, Optional ByVal add_pos = -1)
	...
	If (add_data Is Nothing) Then Call ThrowArgumentNullException(Me, FUNCTION_NAME, "add_data")
	On Error Goto CatchErr
		If (IsOutOfColumnRange(add_pos)) Then add_pos = Me.EndColumnNum
...
Public Function AddRow(ByRef add_data As TableDataRow, Optional ByVal add_pos = -1)
	...
	If (add_data Is Nothing) Then Call ThrowArgumentNullException(Me, FUNCTION_NAME, "add_data")
	On Error Goto CatchErr
		If (IsOutOfRowRange(add_pos)) Then add_pos = Me.EndRowNum
...
Function Initialize(Optional ByVal num_model As Long = 1, Optional ByVal row_size As Long = 1, Optional ByVal table_name As String = "")
	...
	On Error GoTo CatchErr
		If (num_model < 1) Then num_model = 1
		If (row_size < 1) Then row_size = 1
		If (table_name = "") Then table_name = CHECK_LIST_SHEET_TITLE
...
Public Function CopyTitle(ByRef table_data As CheckListData)
	...
	If (table_data Is Nothing) Then	Call ThrowArgumentNullException(Me, FUNCTION_NAME, "table_data", "タイトルのコピーに失敗しました")
...
Public Property Set TableRows(ByVal row As Long, ByRef table_range As TableDataRow)
	...
	If (table_range Is Nothing) Then Call ThrowArgumentNullException(Me, FUNCTION_NAME, "table_range", "コピーに失敗しました")
	If (table_range.Table.Name <> Me.Name) Then Call ThrowArgumentException(Me, FUNCTION_NAME, "table_range", "参照しているテーブル型が対象外です")
...
Public Property Set TableRange(ByRef head As Position, ByRef tail As Position, ByRef table_range As TableDataRange)
	...
	If (table_range Is Nothing) Then Call ThrowArgumentNullException(Me, FUNCTION_NAME, "table_range", "コピーに失敗しました")
	If (table_range.Table.Name <> Me.Name) Then Call ThrowArgumentException(Me, FUNCTION_NAME, "table_range", "参照しているテーブル型が対象外です")
	If (IsHeadOverTail(head, tail)) Then Call ThrowArgumentOutOfRangeException(Me, FUNCTION_NAME, "head/tail", GetMsgHeadOverTailError(head, tail))
	...
	On Error GoTo CatchErr
		...
		Else
			...
			If (Me.NumModel <= tail.column) Then
				Set Me.Range(ToPosition(head.column - Me.NumModel, head.row), ToPosition(tail.column - Me.NumModel, tail.row)) _
					= wrapper.RowsRange(temp, table_range.SourceHead.row, table_range.SourceTail.row)
			End If
		End If
	Exit Property
	CatchErr:
